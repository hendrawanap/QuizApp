...
class HomeViewModel : ViewModel() {
    ...
    private val _user = MutableLiveData<User>().apply{
        ...
        if (user != null) {
            value = User(user.displayName!!, user.photoUrl.toString(), user.email!!)
        }
    }
    ...
    init{
        val user1: UserLeaderboard = UserLeaderboard(R.mipmap.ic_launcher_round, 123)
        val user2: UserLeaderboard = UserLeaderboard(R.mipmap.ic_launcher_round, 103)
        val user3: UserLeaderboard = UserLeaderboard(R.mipmap.ic_launcher_round, 113)
